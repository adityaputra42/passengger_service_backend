...
func NewPassengerMealRepository() PassengerMealRepository { return &passengerMealRepository{} }
...
func (r *passengerMealRepository) FindByPassengerID(ctx context.Context, id uuid.UUID) ([]models.PassengerMeal, error) {
	...
	if err := db.DB.WithContext(ctx).Preload("Meal").Where("passenger_id = ?", id).Find(&meals).Error; err != nil {
...
func (r *passengerMealRepository) Delete(ctx context.Context, id uuid.UUID) error {
	return db.DB.WithContext(ctx).Delete(&models.PassengerMeal{}, "id = ?", id).Error
}
